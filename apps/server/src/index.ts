...
// Email verification redirect page - opens app via deep link
app.get("/app/verify-email", (c) => {
	const token = c.req.query("token");

	if (!token) {
		return c.text("Invalid verification link: missing token", 400);
	}
	...
	// Use Expo Go URL in development, custom scheme in production
	const deepLink = env.EXPO_GO_URL
		? `${env.EXPO_GO_URL}/--/verify-email?token=${encodedToken}`
		: `simple-commerce://verify-email?token=${encodedToken}`;
...
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Verify Email - Simple Commerce</title>
	<style>
		* {
			margin: 0;
			padding: 0;
...
<body>
	<div class="container">
		...
		<p>Tap the button below to open the app and verify your email address.</p>
		<a href="${deepLink}" class="button">Open App</a>
		<div class="fallback">
			<p>If the button doesn't work, copy this token and paste it in the app:</p>
			<div class="token-box">${token}</div>
